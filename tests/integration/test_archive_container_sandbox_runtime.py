...
def test_provisioned_wrapper_pipeline_streams_bounded_tar_without_mounting_inner(
    tmp_path: Path,
) -> None:
    if sys.platform != "linux":
        pytest.skip("native Windows archive sandbox is intentionally unavailable")
    ...
    expected_tar = gzip.decompress(source_bytes)
    ...
    assert result.status is ArchiveContainerRunStatus.COMPLETED
    assert result.inner_stream_size_bytes == len(expected_tar)
    assert result.inner_stream_sha256 == hashlib.sha256(expected_tar).hexdigest()
