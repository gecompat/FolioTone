...
Verifikation und append-only Einzelentscheidungen bleiben jedoch blockiert,
bis `DEC-0001` die `UNBASELINED`-Discovery und die konkrete Fixity-Review-
Lineage ergänzt. Bevorzugt sind der neueste abgeschlossene Scan-Snapshot und
die generische Review-Core-Paarung `FIXITY_EXPECTATION`/`FIXITY_RESULT`.
Danach folgt die Produktoberfläche. Erst nach deren Abschluss
