...
**Status:** In Ausführung; S-EBA-01 bis S-EBA-07 sowie S-EBAR-01 bis
S-EBAR-03 abgeschlossen; FG-A-RUNTIME-AVAILABILITY durch ADR-0041 akzeptiert
...
| FG-A-FORMAT-LOCK | Docs-only Review des durch S-EBAR-02B beobachteten Feldmanifests. | Exakte direkte Containerprofile, äußere Streamprofile, Pflicht-/optionale/verworfene Felder, Leersemantik und Stale-Grenzen werden akzeptiert; kein Produktionscode. | Sol `high`; kein niedrigeres Fallback. |
| S-EBAR-02C | Additive formatgebundene Produktionsparser nach FG-A-FORMAT-LOCK. Erlaubt: `src/foliotone/archive/sevenzip_slt.py`, `src/foliotone/archive/sevenzip.py` und die bestehenden fokussierten Parser-/Command-Tests. | Signaturgebundenes Routing, exakte Golden-Feldprofile, sofortiger Private-Value-Discard und unveränderte v1/v2-Regression. | Terra `medium`; bei Link-, Stream- oder Privacysemantik Sol `high`. Stop bei einer Manifestabweichung. |
| EBAR-05 | Reales unverschlüsseltes Listing und Integrity nach S-EBAR-02C. Erlaubt: neue Datei `src/foliotone/archive/provider.py`, `src/foliotone/archive/workflow.py`, eine Unit- und eine Integrationstestdatei. | Die durch FG-A-FORMAT-LOCK freigegebenen direkten Containerprofile prüfen die konservative Status-/Exitcodematrix aus ADR-0043, getrennte Execution-Provenance, inkrementellen Raw-Discard und unveränderte Sourcebytes. Äußere komprimierte Streams erzeugen vor EBAR-06 keine inneren Member. | Terra `medium`, bei schichtübergreifender Diagnose `high`; Fallback 5.4. Stop bei Verschlüsselung, Manifestabweichung oder ungeklärter Toolfehlermatrix. |
...
S-EBAR-01 bis S-EBAR-03A, EBAR-04 und S-EBAR-02A sowie FG-A-IMAGE und
FG-A-RUNTIME-AVAILABILITY sind abgeschlossen. Die reale Golden-Prüfung von
Parser v2 hat den durch ADR-0043 vorgesehenen Stop ausgelöst. Als Nächstes
folgen deshalb S-EBAR-02B, FG-A-FORMAT-LOCK und S-EBAR-02C; erst danach werden
EBAR-05 und EBAR-06 fortgesetzt. Anschließend legt FG-A-PERSISTENCE den exakten
