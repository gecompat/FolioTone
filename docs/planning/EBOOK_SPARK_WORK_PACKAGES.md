...
| EBAR-05 | Reales unverschlüsseltes Listing und Integrity nach S-EBAR-02A. Erlaubt: neue Datei `src/foliotone/archive/provider.py`, `src/foliotone/archive/workflow.py`, eine Unit- und eine Integrationstestdatei. | Generierte synthetische Archive prüfen die konservative Status-/Exitcodematrix aus ADR-0043, getrennte Execution-Provenance, inkrementellen Raw-Discard und unveränderte Sourcebytes. | Terra `medium`, bei schichtübergreifender Diagnose `high`; Fallback 5.4. Stop bei Verschlüsselung oder ungeklärter Toolfehlermatrix. |
...
S-EBAR-01 bis S-EBAR-03 und FG-A-IMAGE sind abgeschlossen;
FG-A-RUNTIME-AVAILABILITY ist durch ADR-0041 akzeptiert. Als Nächstes setzt
S-EBAR-03A den Acceptance-, Provisioning- und Offline-Availability-Vertrag um;
erst danach folgen EBAR-04 bis EBAR-06 in dieser Reihenfolge. Danach legt
