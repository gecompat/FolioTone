...
| FG-A-EXTRACTION-QUOTA | Separates Docs-only-Frontier-Gate für eine atomar erzwungene Linux-Workspace-Gesamtgrenze vor S-EBAR-04Q. | Entscheidet vorzugsweise einen administrativ vorprovisionierten noexec/nosuid/nodev Quota-Slot-Pool sowie Byte- und Inode-/Member-Caps, freie Reserve, Ownership, Parallelität, Authority-Attestation, Recovery/Cleanup und Kernel-/Tool-Supply-Chain. Polling und `RLIMIT_FSIZE` allein genügen nicht; bis Annahme bleibt Extraction `TOOL_UNAVAILABLE`. | Sol `high`; kein niedrigeres Fallback. Stop bei ungepinnten Hosttools, benötigter Runtime-Mount-Authority/root/CAP_SYS_ADMIN, zwischen Scans überschreitbarem Budget oder nicht beweisbarer Cleanup-/Recoverygrenze. |
| S-EBAR-04Q | Mechanische Implementierung des durch FG-A-EXTRACTION-QUOTA akzeptierten Capability-, Provisionierungs-, Attestations- und Recoveryvertrags. Die exakte kleinste Datei-/Testgrenze wird erst vom Gate festgelegt. | Liefert ausschließlich eine unprivilegiert leasebare opaque Quota-Slot-Capability mit per-run Attestation, leerer Revalidation, Return und Quarantäne/Recovery bei jedem unsicheren Zustand; kein Container- oder Extraction-Lifecycle. Kein spontanes fallocate/mkfs/losetup/mount und keine Runtime-Ausweitung auf root/CAP_SYS_ADMIN. | Vom Gate festzulegen; kein niedrigeres als Sol `high` für neue Privilege-/Mount-/Kernelentscheidungen. Stop ohne realen Cap-/Überlauf-/Return-/Recoverynachweis. |
| S-EBAR-04A | Privater Runner-owned Workspace-Consumer-Lifecycle nach akzeptiertem FG-A-EXTRACTION-QUOTA und abgeschlossenem S-EBAR-04Q. Erlaubt ausschließlich: `src/foliotone/archive/container_sandbox.py`, `tests/unit/test_archive_container_sandbox.py`, `tests/integration/test_archive_container_sandbox_runtime.py`. Die öffentliche `run`-Grenze weist Extraction ab. | Konsumiert nur die unprivilegierte Quota-Slot-Capability; per-run Attestation, zusätzlicher Polling-Frühabbruch und `RLIMIT_FSIZE`, vollständiger Kill/Remove, bewiesene Container-Abwesenheit vor genau einem synchronen S-EBAR-06A-Consumer, Cleanup, leerer Slotbeweis, Return/Quarantäne und Evidence-Freigabe erst danach. Ein realer Linux-Test ist verpflichtend. | Sol `high`; Fallback 5.5 nur ohne neue Sandboxentscheidung. Stop bei frei injizierbarem Callback/Pfad, überlebender Capability, nicht unterscheidbarem Abbruchgrund oder nicht beweisbarer Cap-/Container-/Cleanup-/Return-Reihenfolge. |
...
| FG-A-WRAPPER-PIPELINE | Separates späteres Frontier-Gate für gzip-/bzip2-/xz-/zstd-Wrapper; kein Bestandteil von EBAR-06. | Bounded äußere Dekompression, Byte-/Hash-Lineage, erneute Signature-v2-Prüfung, bestätigtes inneres TAR, getrennte Execution-Provenance, zweiphasige Live-Budgets und Cleanup sowie unverändertes `max_nested_depth=0`. Bis zur Annahme bleiben alle Wrapper `OUTER_COMPRESSION_ONLY`, Storage `UNKNOWN`, `runtime_authorized=false` und ohne Lauf. | Sol `high`; kein niedrigeres Fallback. Stop bei rekursivem Wrapperbedarf, ungeklärter Toolauthority oder nicht beweisbarer doppelter Cleanupgrenze. |
...
FG-A-EXTRACTION-LIFECYCLE ist durch ADR-0048 entschieden; als Nächstes folgen
S-EBAR-05A, S-EBAR-06A, FG-A-EXTRACTION-QUOTA, S-EBAR-04Q, S-EBAR-04A und
danach EBAR-06 für direkte Storage-Familien.
Anschließend legt FG-A-PERSISTENCE den exakten Schema- und Writer-Vertrag
fest; erst dann darf S-EBAR-07 beginnen. FG-A-WRAPPER-PIPELINE und
FG-A-SECRET können separat später folgen und blockieren die direkte
