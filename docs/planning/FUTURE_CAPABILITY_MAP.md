...
`WI-0003` ist implementiert; scan-snapshotgebundene Verifikation und generisch
reviewte Einzelentscheidungen sind `NEXT`. Root-/Replica-Rollen,
Backup-Vergleich und Restore-Evidence bleiben außerhalb dieser ersten
Version.
...
| Fixity Monitoring | unerwartete Byteänderungen in genau einem E-Book-Root erkennen und einzeln reviewen | Baseline implementiert; `WI-0003` Verifikation/Einzelentscheidungen `NEXT` | `DEC-0001`, `WI-0003` (`FUT-009`) | Scan-Snapshot, generischer Fixity-Review und append-only Erwartungsrevisionen, danach Surface |
