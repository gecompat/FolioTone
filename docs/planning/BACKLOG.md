...
|---|---|---|
| NOW | `GATE-0001` | `WI-0003` ist mit Baseline, scan-snapshotgebundener Verifikation, generischen Fixity-Reviews, append-only Einzelentscheidungen und gemeinsamer Application-/CLI-/REST-/Browser-Bedienung abgeschlossen. |
...
| NEXT WAVE | `GATE-0001` | Qualifiziere genau ein festes netzloses EPUB-3-Transformationsprofil mit synthetischen Fixtures, aktueller Tool-/Lizenz-/Security-Prüfung und exakter Byte-Reproduzierbarkeit. |
| LATER | Nach positivem `GATE-0001`: `WI-0004`, danach W4 | Die EPUB-Transformation bleibt bis zum positiven Gate blockiert; Music bleibt die nächste vollständige Mediendomäne nach ausdrücklicher Aktivierung. |
| DECISION | `GATE-0001`, `FG-W10-SIDECAR-WRITE`, `FG-W10-EXTERNAL-LIBRARY-WRITE`, `FG-W10-REORGANIZE`, `FG-W10-ARCHIVE-REWRITE`, W10-003, W10-004 | `DEC-0001` ist für den Verifikations-Slice vollständig entschieden. `DEC-0002` bleibt bis zur ToolProvider-, Byte-Reproduzierbarkeits-, Security- und Lizenzqualifikation nur `Proposed`. |
| BLOCKED | `WI-0004`, `FG-A-SECRET`, `FG-A3-MEMBER-BYTE` | EPUB-Transformation wartet nach Abschluss von Fixity auf `GATE-0001`. Secretkanal und Archive-Member-Byte-Identity bleiben unabhängig blockiert. |
...
| GATE-0001 | NEXT | Qualifiziere nach `WI-0003` genau ein festes netzloses Transformationsprofil mit synthetischen Fixtures, aktueller Tool-/Lizenz-/Security-Prüfung und exakter Byte-Reproduzierbarkeit. |
