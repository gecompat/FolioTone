...
|---|---|---|
| NOW | Surface-Slice von `WI-0003` (`FUT-009`) | Baseline, scan-snapshotgebundene Verifikation, generische Fixity-Reviews und append-only Einzelentscheidungen sind implementiert. Es fehlt nur die gemeinsame Application-/CLI-/REST-/Browser-Bedienung. |
...
| NEXT WAVE | `WI-0003` Surface | Implementiere manuelle persistente Jobs und dieselben path-freien Application-/CLI-/REST-/Browser-Verträge für Baseline, Verifikation und reviewpflichtige Einzelentscheidungen. |
| LATER | `GATE-0001`, danach `WI-0004` und W4 | Erst nach der Fixity-Surface darf das EPUB-Transformationsgate beginnen; Music bleibt die nächste vollständige Mediendomäne nach ausdrücklicher Aktivierung. |
...
| WI-0003 (`FUT-009`) | NEXT | Baseline und Verifikation sind implementiert: Migrationen `0035`/`0036`, rootweites Fencing, maximal zwei frische no-follow Hash-Worker, bounded Snapshot-Worksets, feste generische Fixity-Reviews und append-only Einzelentscheidungen. Als letzter Slice folgt die getrennte Application-/CLI-/REST-/Browser-Surface. |
