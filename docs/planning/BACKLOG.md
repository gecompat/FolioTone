...
|---|---|---|
| NOW | `DEC-0002` | `GATE-0001` ist negativ abgeschlossen. Vor weiterer Transformationsentwicklung muss die kanonische Verpackungs-/ToolProvider-Richtung ausdrücklich entschieden werden. |
...
| NEXT WAVE | — | Keine Transformationswave ist freigegeben, bis `DEC-0002` zwischen kanonischer FolioTone-Verpackung nach calibre, nativem OPF-Patch plus Verpackung oder einem anderen ToolProvider entscheidet. |
| LATER | Nach neuer positiver Profilqualifikation: `WI-0004`, danach W4 | Die EPUB-Transformation bleibt bis zu einem positiven Folgegate blockiert; Music bleibt die nächste vollständige Mediendomäne nach ausdrücklicher Aktivierung. |
| DECISION | `DEC-0002`, `FG-W10-SIDECAR-WRITE`, `FG-W10-EXTERNAL-LIBRARY-WRITE`, `FG-W10-REORGANIZE`, `FG-W10-ARCHIVE-REWRITE`, W10-003, W10-004 | Das rohe calibre-9.13.0-Profil ist trotz vollständig erhaltener Serienprojektion wegen Zeit-/ZIP-Drift nicht byte-reproduzierbar. `DEC-0002` benötigt eine ausdrückliche Folgerichtung. |
| BLOCKED | `WI-0004`, `FG-A-SECRET`, `FG-A3-MEMBER-BYTE` | EPUB-Transformation wartet auf die Entscheidung und ein positives Folgegate. Secretkanal und Archive-Member-Byte-Identity bleiben unabhängig blockiert. |
...
| ID | Status | Item |
|---|---|---|
| W3-001 | DONE | Evaluate calibre 9.13.0, EPUBCheck 5.3.0, Poppler 26.07.0 and qpdf 12.4.0 before native parsers; document roles, deferrals, licenses and the current calibre 9.12.0 security floor. |
...
| S-W10-RN01 | DONE | Die nicht mutierenden Befehle `ebook-rename-propose`, `ebook-rename-preview --private-details`, `ebook-rename-review` und `ebook-rename-plan` verwenden den bestehenden W9-Recipe-/Review-Store. Der Ziel-Basename kommt bounded und ungeloggt über `stdin`; eine opaque Dependency-Scope-ID bindet explizite aktuelle Coverage, bestehende Beziehungen überschreiben falsches `NOT_APPLICABLE`, und bloß fehlende Zeilen bleiben `UNKNOWN`. Es gibt keine Migration, Capability oder Source-Mutation. |
| S-W10-RN02 | DONE | Reine content-addressed Preparation-/Authorization-/Run-/Event-Verträge, owner-only Rename-Capability, erfolgreicher persistenter Conformance-Probe, additive insert-only Migration `0031_ebook_rename_operations`, `EBOOK_RENAME_PREPARATION`-/`EBOOK_RENAME_RUN`-Leases und SQLite-read-only Status sind umgesetzt. Authorization gilt höchstens 15 Minuten und genau einmal; Plan, aktuelle Scan-/Review-/Dependency-Scope-Lineage, Locator-/Source-/Target-/Backend-/Probe-Binder, Fence, gapless Journal, Privacy und Downgrade-Sperre werden fail-closed geprüft. Es gibt keinen Executor und keine neue CLI-Mutationsfläche. |
| S-W10-RN03 | DONE | Das feste Linux-x86_64-glibc-Backend verwendet Raw-`openat2` mit beneath/no-follow/no-xdev und Raw-`renameat2(RENAME_NOREPLACE)` relativ zu demselben gehaltenen Parent-FD. Ein privater Zufallsfixture-Probe, bounded Xattr-Digest, unmittelbar erneuerte Plan-/Capability-/Probe-/Fence-Binder, Source-`fsync`, Parent-`fsync`, Inode-/Byte-/Attributverifikation und die feste Exact-State-Recovery sind intern umgesetzt. Copy+Delete, Overwrite, Shell, externe Tools und CLI besitzen keinen Einstieg oder Fallback. |
...
| DEC-0002 | DECISION | Vorgeschlagener Vertrag für genau EPUB 3 nach EPUB 3, reviewte Metadaten, einen getrennten verwalteten Output-Root, byteidentischen Dry Run/Replay und Einzel-Publish. Das negative `GATE-0001` erteilt keine W10-Authorization; Verpackungs-/ToolProvider-Richtung und neues Profilgate sind offen. |
| GATE-0001 | DONE | calibre 9.13.0 mit festem `ebook-polish --opf`-Profil und vollständigem Metadatensnapshot erhält die projizierte Serie, scheitert aber an exakter Byte-Reproduzierbarkeit; der pfadfreie Nachweis liegt unter `docs/quality/GATE_0001_EPUB_TRANSFORM_QUALIFICATION.md`. |
| WI-0004 | BLOCKED | Implementiere erst nach einer ausdrücklichen Folgerichtung in `DEC-0002` und einem positiven neuen Profilgate den W9-Metadaten-/Transformationssnapshot, Dry Run, operation-spezifische Authority, Publish/Recovery, CLI sowie bounded Batch-Preparation mit Einzelreview in REST/Browser. |
...
| WI-0004 (`FUT-008`) | BLOCKED | W9-007 hat den dauerhaft nicht ausführbaren `FORMAT_TRANSFORM`-Rezeptvertrag abgeschlossen. Nach dem negativen `GATE-0001` wartet die ausführbare EPUB-3-Ableitung auf eine ausdrückliche Folgerichtung, ein positives neues Profilgate und den Accepted-Status von `DEC-0002`. |
| WI-0003 (`FUT-009`) | DONE | Migrationen `0035` bis `0037`, rootweites Fencing, maximal zwei frische no-follow Hash-Worker, bounded Snapshot-Worksets, feste generische Fixity-Reviews, append-only Einzelentscheidungen und die gemeinsame Application-/CLI-/REST-/Browser-Surface sind implementiert. Manuelle persistente Jobs bleiben read-only gegenüber Source Media. |
...
| FUT-011 | DONE | ADR-0067 akzeptiert `local-single-operator/v1`: gemeinsame Application-Verträge für CLI/REST/Worker, getrennte E-Book-/Musik-/Bilder-Einstiege, loopback-only same-origin REST-/Browser-Shell, Username/Passwort mit lokalem One-time-Bootstrap, zeitbegrenzte Private-/Operator-Grants, OpenAPI, Keyset-Pagination, Privacy, Audit, dauerhafte Jobs und einen vom Webprozess getrennten Operator-Worker. Nur E-Books werden aktiviert; jede W10-Oberfläche bleibt operation-spezifisch. Die Endbenutzerdokumentation trennt Schnellstart, umfassendes GUI-Handbuch, gemeinsame Installation für Docker/Podman Compose oder natives Python und eine eigenständige CLI-Referenz; fünf synthetische Browseraufnahmen zeigen den Workflow ohne private Daten. Der Suchrenderer bildet die öffentlichen API-`hits` einschließlich Komponentenstatus ab. Das read-only Compose-Basisprofil ist ohne Writer-Secrets startbar; der Rename-Operator bleibt im fail-closed Overlay. Die `WI-0003`-Fixity-Surface ergänzt manuelle read-only Jobs, Status, private Projektionen und Einzelreview ohne W10- oder Source-Write-Ausweitung. |
