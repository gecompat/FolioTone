...
|---|---|---|
| NOW | `W10-005` | Nach der vollständigen EPUB-Titelwriter-Kette vervollständigt der nächste getrennte Slice die bereits durch ADR-0056 erlaubte Interim-Ein-Datei-Quarantäne mit Authorize, Execute, Recovery und zweiter Bestätigung. |
...
| NEXT WAVES | `W10-005` | Der private Capability Resolver ist vorhanden; CLI und vollständige Recovery-Bedienkette der Quarantäne fehlen noch. `FG-W10-MOVE-BACKEND` bleibt davon getrennt. |
...
| W10-005 | NEXT | Vervollständige nach der vorhandenen `quarantine-authorize`-CLI die ADR-0056-Bedienkette mit Execute, zweiter Bestätigung, One-use-Fencing und Recovery in getrennten kleinen Folgepaketen. CLI-Argumente enthalten nur opaque IDs und Content Hashes; die zweite Bestätigung läuft ausschließlich über nicht geloggtes `stdin`. Der Slice bleibt bei genau einer regulären Same-Filesystem-Datei und behauptet keine atomare No-Replace-Garantie. |
...
| S-W10-05C | NEXT | Ergänze `quarantine-execute` mit erneutem opaque Binder-Check, exakt einer begrenzten nicht geloggten `stdin`-Bestätigung, Authorization-One-use, unmittelbarer Plan-/Source-/Capability-Revalidierung, `CONSOLIDATION_QUARANTINE_RUN`-Lease und Aufruf ausschließlich des vorhandenen Interim-Executors. |
| S-W10-05D | PLANNED | Ergänze `quarantine-recover` für genau eine opaque Run-ID, die feste ADR-0056-Recovery-Matrix und die synthetische Crash-/Recovery-End-to-End-Abnahme; unklare Zustände enden ohne weitere Mutation bei `MANUAL_REVIEW`. |
