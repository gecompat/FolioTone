...
|---|---|---|
| NOW | `GATE-0002` | Option A ist durch `DEC-0002` akzeptiert. Das kombinierte calibre-/FolioTone-Profil muss jetzt ausschließlich synthetisch und output-only positiv qualifiziert werden. |
...
| NEXT WAVE | `GATE-0002` | Vollständigen Transformations-Metadaten-Snapshot, kanonische OPF-Normalisierung und EPUB-Verpackung mit exaktem Byte-Replay qualifizieren. |
| LATER | Nach positivem `GATE-0002`: `WI-0004`, danach W4 | Die EPUB-Transformation bleibt bis zu einem positiven Folgegate blockiert; Music bleibt die nächste vollständige Mediendomäne nach ausdrücklicher Aktivierung. |
| DECISION | `FG-W10-SIDECAR-WRITE`, `FG-W10-EXTERNAL-LIBRARY-WRITE`, `FG-W10-REORGANIZE`, `FG-W10-ARCHIVE-REWRITE`, W10-003, W10-004 | Diese unabhängigen Operations- und Produktgrenzen benötigen weiterhin eigene Entscheidungen. |
| BLOCKED | `WI-0004`, `FG-A-SECRET`, `FG-A3-MEMBER-BYTE` | EPUB-Transformation wartet auf ein positives `GATE-0002`. Secretkanal und Archive-Member-Byte-Identity bleiben unabhängig blockiert. |
...
| DEC-0002 | DONE | Option A ist akzeptiert: calibre 9.13.0 erhält einen vollständigen Transformations-Metadaten-Snapshot; eine FolioTone-eigene kanonische OPF-Normalisierung und EPUB-Verpackung verarbeitet nur den privaten Tooloutput. Die Entscheidung öffnet keine Writer-, Publish- oder W10-Capability. |
...
| GATE-0002 | READY | Qualifiziere das kombinierte Option-A-Profil mit getrennten Review-/Preserve-Werten, exakt einem technischen `dcterms:modified`-Delta, kanonischer ZIP-/OPF-Serialisierung, frischem Byte-Replay, Idempotenz, Malicious Fixtures und unabhängigen Preserved-Field-/Nutzdaten-Read-backs. |
| WI-0004 | BLOCKED | Implementiere erst nach positivem `GATE-0002` den W9-Metadaten-/Transformationssnapshot, Dry Run, operation-spezifische Authority, Publish/Recovery, CLI sowie bounded Batch-Preparation mit Einzelreview in REST/Browser. |
...
| WI-0004 (`FUT-008`) | BLOCKED | W9-007 hat den dauerhaft nicht ausführbaren `FORMAT_TRANSFORM`-Rezeptvertrag abgeschlossen. `DEC-0002` akzeptiert Option A; die ausführbare EPUB-3-Ableitung wartet weiterhin auf ein positives `GATE-0002`. |
