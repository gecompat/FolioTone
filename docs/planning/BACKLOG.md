...
|---|---|---|
| NOW | Detailentscheidung für den Verifikations-Slice von `WI-0003` (`FUT-009`) | Der Baseline-Slice ist implementiert. `UNBASELINED`-Discovery und die konkrete Fixity-Review-Lineage sind in `DEC-0001` noch nicht vollständig entschieden. |
...
| NEXT WAVE | Keine bis zur Detailentscheidung | Der Verifikations-Slice darf erst nach ausdrücklicher Auswahl des Discovery- und Reviewvertrags fortgesetzt werden. |
...
| DECISION | `DEC-0001`-Verifikationsdetails, `GATE-0001`, `FG-W10-SIDECAR-WRITE`, `FG-W10-EXTERNAL-LIBRARY-WRITE`, `FG-W10-REORGANIZE`, `FG-W10-ARCHIVE-REWRITE`, W10-003, W10-004 | Für Fixity sind Scan-Snapshot oder eigene Discovery sowie generischer oder dedizierter Reviewvertrag auszuwählen. `DEC-0002` bleibt bis zur ToolProvider-, Byte-Reproduzierbarkeits-, Security- und Lizenzqualifikation nur `Proposed`. |
| BLOCKED | `WI-0003`, `WI-0004`, `FG-A-SECRET`, `FG-A3-MEMBER-BYTE` | Fixity wartet auf die Detailentscheidung; EPUB-Transformation danach auf `GATE-0001`. Secretkanal und Archive-Member-Byte-Identity bleiben unabhängig blockiert. |
...
| WI-0003 (`FUT-009`) | BLOCKED | Der Baseline-Slice implementiert immutable Build-/Entry-/Manifest-/Aktivierungsverträge, Migration `0035`, rootweites Fencing, maximal zwei frische no-follow Hash-Worker und eine echte SQLite-`query_only`-Projektion. Verifikation/Einzelentscheidungen warten auf die in `DEC-0001` dokumentierte Auswahl für `UNBASELINED`-Discovery und Fixity-Review-Lineage; Surface folgt erst danach. |
