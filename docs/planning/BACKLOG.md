...
|---|---|---|
| NOW | `S-W10-RN03` | RN02 liefert die nicht ausführende Rename-Authority samt Capability, Probe, Fencing, insert-only Journal und read-only Status. Als nächster kleiner Slice folgt ausschließlich das feste Linux-No-Replace-Backend mit unmittelbarer Verifikation und Exact-State-Recovery; CLI und Reconciliation bleiben geschlossen. |
...
| NEXT WAVES | `S-W10-RN03` -> `S-W10-RN04` | Linux-No-Replace-Backend/Recovery und danach Bedien-/Scan-/Reconciliation-Kette; jede Wave bleibt für sich operation-spezifisch begrenzt. |
...
| S-W10-RN03 | NEXT | Implementiere das feste Linux-x86_64-glibc-Backend mit `openat2` und genau einem `renameat2(RENAME_NOREPLACE)`, unmittelbarer Inode-/Byte-/Attributverifikation, Parent-`fsync` und der ADR-0066-Exact-State-Recovery; keine CLI und kein Fallback. |
