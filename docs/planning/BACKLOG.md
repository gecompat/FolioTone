...
| W3-016 | NEXT | Add deterministic CLI collection reports with aggregate format/analysis/quality/finding counts plus prioritized incomplete, action-required, duplicate and variant review sets. Keep private paths and metadata in local runtime artifacts and out of Git. |
| W3-017 | PLANNED | Run a read-only pilot and then the complete confirmed local collection analysis through the host context after W3-015/W3-016; record scale, runtimes, failure classes and actionable collection findings without changing source media or committing private paths/data. |
