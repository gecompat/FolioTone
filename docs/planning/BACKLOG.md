...
|---|---|---|
| NOW | `S-W10-RN04` | RN03 liefert das feste interne Linux-No-Replace-Backend, den gefenceten Executor, unmittelbare Verifikation und Exact-State-Recovery. Als letzter enger Rename-Slice folgt ausschließlich die Bedien-/Scan-/Reconciliation-Kette; andere Operationsarten bleiben geschlossen. |
...
| NEXT WAVES | `S-W10-RN04` | Feste Authorize-/Execute-/Recover-/Status-Bedienung, Lease-Handoff, Folgescan, `CollectionState` und immutable Reconciliation schließen nur das enge Same-Parent-Rename-Profil ab. |
...
| S-W10-RN04 | NEXT | Ergänze feste Authorize-/Execute-/Recover-/Status-Kommandos, zweite `stdin`-Bestätigung, Lease-Handoff, neuen Scan, `CollectionState`, Migration `0032_ebook_rename_reconciliation` und den immutable `EbookRenameReconciliationSnapshot`; erst diese Wave macht das enge Profil operativ erreichbar. |
