...
| W3-013 | NEXT | Implement provider-neutral e-book comparison over persisted file, text, metadata, structure and cover Evidence; do not depend on calibre's GUI-only diff and do not turn similarity into canonical identity. |
...
| W3-015 | PLANNED | Add resumable collection-batch orchestration over persisted current EPUB/MOBI/AZW/AZW3/PDF observations. Reuse exact Evidence, continue after per-file failures, bound concurrency and preserve read-only source handling. |
| W3-016 | PLANNED | Add deterministic CLI collection reports with aggregate format/analysis/quality/finding counts plus prioritized incomplete, action-required, duplicate and variant review sets. Keep private paths and metadata in local runtime artifacts and out of Git. |
| W3-017 | PLANNED | Run a read-only pilot and then the complete collection analysis for `Z:\NAS\E-Book` once the source is visible to the Codex process; record scale, runtimes, failure classes and actionable collection findings without changing source media. |
