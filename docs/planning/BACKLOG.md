...
| W3-017 | IN PROGRESS | Harden incremental scan/hash persistence from real read-only scale evidence; add format-balanced pilot planning, resumable full-SHA enrichment limited to Quick duplicate candidates and a deterministic scan-wide inventory/hash/duplicate report; complete the private collection analysis/report without changing source media or committing private paths, counts, or runtime data. |
