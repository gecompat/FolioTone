...
|---|---|---|
| NOW | `WI-0004` | `GATE-0002` hat das kombinierte calibre-/FolioTone-Profil positiv qualifiziert. Die EPUB-Transformation beginnt mit dem nicht ausführbaren Planvertrag und dem privaten Dry Run. |
...
| NEXT WAVE | `WI-0004` | Den transform-spezifischen W9-Snapshot und privaten netzlosen Dry Run als kleinsten ersten Vertikalschnitt implementieren; noch keine Authority oder W10-Operation. |
| LATER | Nach `WI-0004`: W4 | Music bleibt die nächste vollständige Mediendomäne nach ausdrücklicher Aktivierung. |
...
| BLOCKED | `FG-A-SECRET`, `FG-A3-MEMBER-BYTE` | Secretkanal und Archive-Member-Byte-Identity bleiben unabhängig blockiert. |
...
| GATE-0002 | DONE | Das kombinierte Option-A-Profil ist positiv qualifiziert: zwei verschiedene rohe calibre-Outputs ergeben nach kanonischer ZIP-/OPF-Serialisierung denselben 2.073-Byte-Output und denselben vollständigen SHA-256; Idempotenz, Serie, OPF-Struktur, Preserved Fields, Nutzdaten und EPUBCheck 5.3.0 sind unabhängig bestätigt. |
| WI-0004 | READY | Implementiere in getrennten Vertikalschnitten zuerst W9-Metadaten-/Transformationssnapshot und privaten Dry Run, danach operation-spezifische Authority, Publish/Recovery, CLI sowie bounded Batch-Preparation mit Einzelreview in REST/Browser. |
...
| WI-0004 (`FUT-008`) | READY | W9-007 hat den dauerhaft nicht ausführbaren `FORMAT_TRANSFORM`-Rezeptvertrag abgeschlossen; `DEC-0002` akzeptiert Option A und `GATE-0002` qualifiziert das kombinierte Profil positiv. Die Implementierung beginnt mit Planvertrag und privatem Dry Run, nicht mit W10-Authority. |
